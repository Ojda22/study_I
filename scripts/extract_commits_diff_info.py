...
from scripts.mutation_comparision import map_mutants, return_tests, calculate_ms, calculate_relevant_ms
    ...
    for _index, row in data_frame.iterrows():
        ...
        for mutants_type in mutants_types:
            mutants_types_dict.append({"Mutant_Type": mutants_type,
                                       "Mutants": len(
                                           [mutant for mutant in relevant_mutants if
                                            mutant.mutant_operator == mutants_type]),
                                       "Minimal_Mutants": len(
                                           [mutant for mutant in minimal_relevant_mutants if
                                            mutant.mutant_operator == mutants_type]),
                                       "All_Mutants": len(
                                           [mutant for mutant in all_granularity_level if
                                            mutant.mutant_operator == mutants_type])})
...
def commit_tests(data_file_path, path_to_mutants_data):
    data_frame = pd.read_csv(data_file_path, index_col="commit", thousands=",")

    projects = {"commons-collections": [0, 0],
                "commons-csv": [0, 0],
                "commons-io": [0, 0],
                "commons-lang": [0, 0],
                "commons-text": [0, 0]
    ...
    for _index, row in data_frame.iterrows():
        ...
        print(_index)
        developer_tests, evosuite_tests = return_tests(
            mutants_info_path=mutantsInfoPath, mutation_matrix_path=mutationMatrixPath)

        projects[row["project"]][0] += len(developer_tests)
        projects[row["project"]][1] += len(evosuite_tests)

    print(projects)

def calculate_mutation_score(data_file_path, path_to_mutants_data, output_path):
    data_frame = pd.read_csv(data_file_path, index_col="commit", thousands=",")
    ...
    for _index, row in data_frame.iterrows():
        ...
        print(_index)
        # developer_MS, evosuite_MS, MS = calculate_ms(
        #     mutants_info_path=mutantsInfoPath, mutation_matrix_path=mutationMatrixPath)
        relevant_MS = calculate_relevant_ms(mutants_info_path=mutantsInfoPath, mutation_matrix_path=mutationMatrixPath)

        # ms_dict.append({"commit":_index, "dev_MS" : developer_MS, "evo_MS" : evosuite_MS, "MS" : MS})
        ms_dict.append({"commit":_index, "relevant_MS" : relevant_MS})

    try:
        with open(output_path + "/minimal_relevant_mutation_score.csv", 'w') as csvfile:
            # writer = csv.DictWriter(csvfile, fieldnames=["commit", "dev_MS", "evo_MS", "MS"])
            writer = csv.DictWriter(csvfile, fieldnames=["commit", "relevant_MS"])
            writer.writeheader()
            for data in ms_dict:
                writer.writerow(data)
    except IOError:
...
#         # print(mutants_types)
#         for mutants_type in mutants_types:
#             mutants_types_dict.append({"Mutant_Type": mutants_type, "Mutants": len(
#                 [mutant for mutant in relevant_mutants if mutant.mutant_operator == mutants_type])})
#
#         for commit in RepositoryMining(
#                 path_to_repo="https://github.com/apache/commons-" + row["project"].split("-")[1] + ".git",
#                 single=_index).traverse_commits():
#             for m in commit.modifications:
...
#                 mutants_in_file_modified = [mutant for mutant in relevant_mutants if
#                                             mutant.sourceFile == m.filename.split("/")[-1]]
...
#         hunks = HunksCount(path_to_repo="https://github.com/apache/commons-" + row["project"].split("-")[1] + ".git",
#                            from_commit=_index, to_commit=_index)
#         hunks_dict = hunks.count()
#
#         for file, hunks_number in hunks_dict.items():
#             file = file.split("/")[-1]
...
#         output_file.write(
#             "Mutant In Method: {} \nMutants Outside Method: {}".format(str(mutants_on_method["In_Method"]),
#                                                                        str(mutants_on_method["Not_In_Method"])))
#
#     try:
...
if __name__ == '__main__':
    ...
    #                            output_path=arguments.output_dir)
    # commit_tests(data_file_path=arguments.statistics_file,
    #              path_to_mutants_data=arguments.path_to_mutants_data)
    calculate_mutation_score(data_file_path=arguments.statistics_file,
                 path_to_mutants_data=arguments.path_to_mutants_data, output_path=arguments.output_dir)
